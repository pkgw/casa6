...
def fillSummaryMinor(residname, field_id, channo, stokes, stokes_axis, freq_axis):
    ...
    field_id        Field ID to query in the image, int
...
def constructSummaryMinor(bparm):
    ...
    being re-written here to avoid issues with iterControl

    Inputs:
    bparm           The tclean inputs, defaults where not specified. dict

    Returns:
    summaryMinor    Nested dictionary with the summaryMinor keys
    """
    ...
    if os.path.exists(residname):
        ...
        # Multi-fields is a bit of question mark, so for now just focusing on single field.
        nfields = len(bparm['field'].split(','))
        casalog.post("Number of input fields in BPARM is %d" % nfields, "INFO3", "task_tclean")
...
def constructResidualDict(bparm):
    """
    ...
    # Check that the residual image exists to interrogate, otherwise return dummy dict

    fill_dummy_values=True

    residname=bparm['imagename']+'.residual.tt0' if(os.path.exists( bparm['imagename']+'.residual.tt0')) else bparm['imagename']+'.residual'
    #if os.path.exists(residname):
    #    fill_dummy_values = False

    retrec = dict()
    # Initialize the values that don't need to inspect any images
    retrec['cleanstate'] = 'running'
    retrec['cyclefactor'] = bparm['cyclefactor']
    retrec['cycleiterdone'] = 0
    retrec['cycleniter'] = 0
    retrec['cyclethreshold'] = 0
    retrec['interactiveiterdone'] = 0
    retrec['interactivemode'] = bparm['interactive']
    ...
    # Construct this in the right way
    retrec['summaryminor'] = constructSummaryMinor(bparm)
    ...
    try: 
        ...
        if niter >=0 : 
            ...
            if niter==0 and calcres==True:
                retrec = constructResidualDict(bparm)
