...
def construct_residual_dict(bparm):
    """
    ...
    # How to fill this?
    retrec['interactiveniterthreshold'] = bparm['threshold']
    ...
    retrec['nmajordone'] = 1
    retrec['nsigma'] = 0.0
    # TODO check for the right stopcode
    retrec['stopcode'] = 1

    if fill_dummy_values:
        retrec['maxpsffraction'] = 0
        retrec['minpsffraction'] = 0
        retrec['maxpsfsidelobe'] = 0
        retrec['summarymajor'] = np.array([0,])
        # Construct this in the right way
        retrec['summaryminor'] = None
        retrec['threshold'] = 0.0
        retrec['stopDescription'] = 'iteration limit'
        pass
    else:
        pass
...
#        casalog.post("pointingoffsetsigdev is set to zero which is an unphysical value, will proceed with the native sky pixel resolution instead". "WARN")
    ...
    try: 
        ...
        if niter >=0 : 
            ...
            # XXX Look at residual image and return dictionary here
            if niter==0 and calcres=True: # Residual image needs to be computed for this to work
                retrec = construct_residual_dict(bparm)
