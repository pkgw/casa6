...
from casatools import imager, ctsys
from casatasks import casalog, impbcor, imdev, imhead, imsubimage, imstat, immath, tclean
...
class test_j1302(StkUnitTest):
    ...
    def test_j1302_awproject(self):
        ...
        #intermediate pipeline step.
        test_name = self._testMethodName
        data_path_dir  = os.path.join(root_data_path, 'J1302/Stakeholder-test-awproject-data')
        cf0 = 'J1302_iter0d'
        cf1 = 'J1302_iter2'
    ...
    # Test 2b
    @stats_dict(test_dict)
    def test_j1302_awp2(self):
        """ J1302 (southern source), awp2 gridder, Stokes I imaging"""
        ######################################################################################
        # Should match values for "Stokes I" in the "Values to be compared"
        ######################################################################################
        ...
        imsize=5250
        self.prepData(self.vis, data_path_dir, *masks, partial_results_dirname="partial_results_test_j1302_awproject")
        ...
        common_args = {'vis':'J1302-12fields.ms', 'uvrange':'<12km', 'intent':'OBSERVE_TARGET#UNSPECIFIED', \
                       'imsize':imsize, 'cell':'0.6arcsec', 'phasecenter':'13:03:13.874 -10.51.16.73', \
                       'spw':'2~17', 'reffreq':'3.0GHz', \
                       'gridder':'awp2', 'wprojplanes':32, 'mosweight':False, 
                       'usepointing':True, 'rotatepastep':5.0, \
                       'pblimit':0.02, 'specmode':'mtmfs_via_cube','nchan':16,\
                       'deconvolver':'mtmfs', 'nterms':2, 'smallscalebias':0.4, \
                       'weighting':'briggs', 'robust':1.0,\
                       'threshold':0.0, 'cyclefactor':3.0, 'restart':True, 'interactive':False, 'parallel':self.parallel}

        def run_tclean(imagename=None, niter=None, datacolumn='corrected', mask='', usemask='user', pbmask=0.0,  nsigma=2.0, cycleniter=5000, scales=[0],
              calcres=False, calcpsf=False, savemodel='none', common_args=common_args):
            params = locals()
            finalparams = self.filter_runtclean_parameters(params, common_args)
            # add images to self.imgs for clean-up
        ...
        # initialize iter2, no cleaning
        run_tclean(imagename=img1,  niter=0, calcres=True, calcpsf=True )
        ...
        # save model column, doesn't happen here in acutal VLASS pipeline, but makes sure functionality works.
        run_tclean(imagename=img1, niter=0, datacolumn='data', savemodel='modelcolumn')
    ...
    # Test 2c
    @stats_dict(test_dict)
    def test_j1302_awphpg(self):
        """ J1302 (southern source), awphpg gridder, Stokes I imaging"""
        ######################################################################################
        # Should match values for "Stokes I" in the "Values to be compared"
        ######################################################################################
        ...
        imsize=5250
        self.prepData(self.vis, data_path_dir, *masks, partial_results_dirname="partial_results_test_j1302_awphpg")
        ...
        common_args = {'vis':'J1302-12fields.ms', 'uvrange':'<12km', 'intent':'OBSERVE_TARGET#UNSPECIFIED', \
                       'imsize':imsize, 'cell':'0.6arcsec', 'phasecenter':'13:03:13.874 -10.51.16.73', \
                       'spw':'2~17', 'reffreq':'3.0GHz', \
                       'gridder':'awphpg', 'wprojplanes':32, 'mosweight':False, 
                       'usepointing':True, 'rotatepastep':5.0, \
                       'pblimit':0.02, 'specmode':'mtmfs_via_cube','nchan':16,\
                       'deconvolver':'mtmfs', 'nterms':2, 'smallscalebias':0.4, \
                       'weighting':'briggs', 'robust':1.0,\
        ...
        # save model column, doesn't happen here in acutal VLASS pipeline, but makes sure functionality works. AWPHPG does not get back model visibilities ...so no model column can be written
        #run_tclean(imagename=img1, niter=0, datacolumn='data', savemodel='modelcolumn')
        ...
        # awphpg does not generate model vis
        # (e) Confirm presence of model column in resultant MS
        #success7, report7 = self.check_column_exists("MODEL_DATA")
        ...
        # TODO start obeying on-axis once on-axis passes
        report  = "".join([report1, report3, report5, report6])
        success = success1 and success3 and success5 and success6 and self.th.check_final(report)
        # report  = "".join([report0, report1, report2, report3, report4, report5, report6, report7])
        # success = success0 and success1 and success2 and success3 and success4 and success5 and success6 and success7 and self.th.check_final(report)
    ...
    def test_j1302_mosaic_cube(self):
        ...
        #intermediate pipeline step.
        test_name = self._testMethodName
        data_path_dir  = os.path.join(root_data_path, 'J1302/Stakeholder-test-mosaic-cube-data')
...
class test_j1927(StkUnitTest):
    ...
    def test_j1927_mosaic_cube(self):
        ...
        #intermediate pipeline step.
        test_name = self._testMethodName
        data_path_dir  = os.path.join(root_data_path, 'J1927/J1927-stakeholdertest-mosaic-cube-data')
