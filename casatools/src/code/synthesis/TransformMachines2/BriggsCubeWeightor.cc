...
BriggsCubeWeightor::BriggsCubeWeightor()
    : grids_p(0), ft_p(0), f2_p(0), d2_p(0), uscale_p(0), vscale_p(0),
      uorigin_p(0), vorigin_p(0), nx_p(0), ny_p(0), rmode_p(""), noise_p(0.0),
      robust_p(2), superUniformBox_p(0), multiField_p(False),
      initialized_p(False), refFreq_p(-1.0),
...
BriggsCubeWeightor::BriggsCubeWeightor(
    const String &rmode, const Quantity &noise, const Double robust,
    const Double &fracBW, const Int superUniformBox, const Bool multiField)
    : grids_p(0), ft_p(0), f2_p(0), d2_p(0), uscale_p(0), vscale_p(0),
      uorigin_p(0), vorigin_p(0), nx_p(0), ny_p(0), initialized_p(False),
      refFreq_p(-1.0),
      freqInterpMethod_p(InterpolateArray1D<Double, Complex>::nearestNeighbour),
...
BriggsCubeWeightor::BriggsCubeWeightor(
    vi::VisibilityIterator2 &vi, const String &rmode, const Quantity &noise,
    const Double robust, const ImageInterface<Complex> &templateimage,
    const RecordInterface &inrec, const Double &fracBW,
...
String BriggsCubeWeightor::initImgWeightCol(
    vi::VisibilityIterator2 &vi, const ImageInterface<Complex> &templateimage,
  ...
  if (initialized_p && nx_p == templateimage.shape()(0) &&
      ny_p == templateimage.shape()(1)) {
  ...
  Double freqend =
      cs.toWorld(IPosition(4, 0, 0, 0, templateimage.shape()[3] - 1))[3];
  ...
  for (vi.originChunks(); vi.moreChunks(); vi.nextChunk()) {
    for (vi.origin(); vi.more(); vi.next()) {
      ...
      if (multiField_p) {
        ...
        if (std::find(fieldsToUse.begin(), fieldsToUse.end(), ms_field) ==
            fieldsToUse.end()) {
  ...
  freqInterpMethod_p =
      static_cast<InterpolateArray1D<Double, Complex>::InterpolationMethod>(
          tmpInt);
  ostringstream oss;
  oss << std::setprecision(12) << nrows << "_" << freqbeg << "_" << freqend
      << "_" << rmode_p << "_" << robust_p << "_interp_" << tmpInt;
  ...
  /////////////////////////////
  if (inOneGo) {
    fillImgWeightCol(vi, inRec, -1, fieldsToUse, allSwingPad,
...
void BriggsCubeWeightor::fillImgWeightCol(
    vi::VisibilityIterator2 &vi, const Record &inRec, const Int msid,
    std::vector<pair<Int, Int>> &fieldsToUse, const uInt swingpad,
    const IPosition &origShp, CoordinateSystem csOrig) {
  vi::VisBuffer2 *vb = vi.getVisBuffer();
  for (uInt k = 0; k < fieldsToUse.size(); ++k) {
    vi.originChunks();
    vi.origin();
    // cerr << "####nchannels " << vb->nChannels() << " swingpad " << swingpad
    // << endl;
    ...
    // cerr << "REFPIX " << refpix << " new SHAPE " << shp  << " swigpad " <<
    // swingpad  << " msid "<< msid << " fieldToUse.msid "
    // <<fieldsToUse[k].first << " fieldid " <<  fieldsToUse[k].second << endl;
    ...
    for (vi.originChunks(); vi.moreChunks(); vi.nextChunk()) {
      for (vi.origin(); vi.more(); vi.next()) {
        // cerr << "key and index "<< key << "   " << index << "   " <<
        // multiFieldMap_p[key] << endl;
        if ((vb->fieldId()[0] == fieldsToUse[k].second &&
             vb->msId() == fieldsToUse[k].first) ||
            fieldsToUse[k].first == -1) {
    ...
    for (uInt chan = 0; chan < uInt(nchan); ++chan) {
      ...
      Matrix<Float> gwt(
          griddedWeight(start, end).reform(IPosition(2, nx_p, ny_p)));
      if ((rmode_p == "norm" || rmode_p == "bwtaper") &&
          (sumWgts(0, chan) >
           0.0)) { // See CAS-13021 for bwtaper algorithm details
        // os << "Normal robustness, robust = " << robust << LogIO::POST;
        Double sumlocwt = 0.;
        for (Int vgrid = 0; vgrid < ny_p; vgrid++) {
          for (Int ugrid = 0; ugrid < nx_p; ugrid++) {
            if (gwt(ugrid, vgrid) > 0.0)
              sumlocwt += square(gwt(ugrid, vgrid));
          }
        }
        f2_p[0][chan] = square(5.0 * pow(10.0, Double(-robust_p))) /
                        (sumlocwt / (2 * sumWgts(0, chan)));
        d2_p[0][chan] = 1.0;
        ...
        if ((msid < 0) || ((vb->msId()) == (msid))) {
          if ((vb->fieldId()[0] == fieldsToUse[k].second &&
            ...
            /*///////////
              ...
              int n=0;
              std::for_each(cmap.begin(), cmap.end(), [&n, &myfile](int
              &val){if(val > -1)myfile << " " << n; n++; }); myfile << "----msid
              " << vb->msId() << endl;
                  /////////////////////////////*/
            ...
            // Slicer sl(IPosition(2,beginrow,0), IPosition(2,endrow,nChans-1),
            // Slicer::endIsLast);
            ArrayColumn<Float> col(*wgtTab_p, "IMAGING_WEIGHT");
            // cerr << "sl length " << sl.length() << " array shape " <<
            // fakeweight.shape() << " col length " << col.nrow() << endl;
            // col.putColumnRange(sl, fakeweight);
            // cerr << "nrows " << nRows << " imweight.shape " <<
            // imweight.shape() << endl;
            for (rownr_t row = 0; row < nRows; ++row)
              col.put(beginrow + row, imweight.column(row));

            Slicer sl2(IPosition(1, endrow - nRows + 1), IPosition(1, endrow),
                       Slicer::endIsLast);
...
Int BriggsCubeWeightor::estimateSwingChanPad(vi::VisibilityIterator2 &vi,
                                             const Int msid,
  ...
  for (auto itmin = localminfreq.begin(); itmin != localminfreq.end();
       ++itmin) {
  ...
  swingpad =
      2 * (Int(std::ceil((swingFreq + firstchanshift) / freqincr)) + extrapad);
  // cerr <<" swingfreq " << (swingFreq/freqincr) << " firstchanshift " <<
  // (firstchanshift/freqincr) << " SWINGPAD " << swingpad << endl;
  ////////////////
  return swingpad;
}
...
void BriggsCubeWeightor::init(vi::VisibilityIterator2 &vi,
  ...
  for (vi.originChunks(); vi.moreChunks(); vi.nextChunk()) {
    for (vi.origin(); vi.more(); vi.next()) {
      String key = String::toString(vb->msId()) + "_" +
                   String::toString(vb->fieldId()(0));
      Int index = 0;
      if (multiField_p) {
  ...
  vi.origin();
  String key =
      String::toString(vb->msId()) + "_" + String::toString(vb->fieldId()(0));
  IPosition shp = templateimage.shape();
  nx_p = shp[0];
  ny_p = shp[1];
  CoordinateSystem cs = templateimage.coordinates();
  ...
  TempImage<Complex> newTemplate(shp, cs);
  ///////////////////////
  // ImageInterface<Complex>& newTemplate=const_cast<ImageInterface<Complex>&
  // >(templateimage);
  ...
  for (int index = 0; index < nIndices; ++index) {
    ...
    // cerr << index << " griddedWeight Shape " << griddedWeight.shape() <<
    // endl;
    grids_p[index]->put(griddedWeight.reform(newTemplate.shape()));
  ...
  for (uInt index = 0; index < f2_p.nelements(); ++index) {
    ...
    for (uInt chan = 0; chan < uInt(nchan); ++chan) {
      ...
      if ((rmode_p == "norm" || rmode_p == "bwtaper") &&
        ...
        f2_p[index][chan] = square(5.0 * pow(10.0, Double(-robust_p))) /
                            (sumlocwt / (2 * sumWgts[index](0, chan)));
        d2_p[index][chan] = 1.0;
...
void BriggsCubeWeightor::weightUniform(Matrix<Float> &imweight,
  ...
  if (rmode_p == "bwtaper") // See CAS-13021 for bwtaper algorithm details
  {
    if (fracBW == 0.0) {
      throw(AipsError(
          "BriggsCubeWeightor fractional bandwith is not a valid value, 0.0."));
    }
      ...
      if ((!flag(chn, row)) && (chanMap(chn) > -1)) {
        ...
        if ((ucell > 0) && (ucell < nx_p) && (vcell > 0) && (vcell < ny_p)) {
          ...
          if (gwt > 0) {
            ...
            if (rmode_p ==
                "bwtaper") { // See CAS-13021 for bwtaper algorithm details
              nCellsBW = fracBW *
                         sqrt(pow(uscale_p * u, 2.0) + pow(vscale_p * v, 2.0));
              uvDistanceFactor = nCellsBW + 0.5;
              if (uvDistanceFactor < 1.5)
                uvDistanceFactor = (4.0 - nCellsBW) / (4.0 - 2.0 * nCellsBW);
              imweight(chn, row) /=
                  gwt * f2_p[index][pos[3]] / uvDistanceFactor +
                  d2_p[index][pos[3]];
            } else {
              imweight(chn, row) /=
                  gwt * f2_p[index][pos[3]] + d2_p[index][pos[3]];
            }
...
void BriggsCubeWeightor::readWeightColumn(
    casacore::Matrix<casacore::Float> &imweight, const vi::VisBuffer2 &vb) {
  ...
  if (vbrowms2wgtrow_p.size() == 0) {
    ...
    // cerr << "Map size " << vbrowms2wgtrow_p.size() << " max size " <<
    // vbrowms2wgtrow_p.max_size() << endl;
  }
  ...
  for (rownr_t k = 0; k < (vb.nRows()); ++k) {
    ...
    // cerr << imwgtcol.get(tabrow).shape() << "   " <<
    // imweight.column(k).shape() << endl;
    imweight.column(k) = imwgtcol.get(tabrow);
  }
}
void BriggsCubeWeightor::getWeightUniform(const Array<Float> &wgtDensity,
                                          Matrix<Float> &imweight,
                                          const vi::VisBuffer2 &vb) {
  ...
  // int n=0;
  // std::for_each(cmap.begin(), cmap.end(), [&n](int &val){if(val > -1)cerr <<
  // " " << n; n++; }); cerr << "----msid " << vb.msId() << endl;
  Int nvischan = vb.nChannels();
  rownr_t nRow = vb.nRows();
  Matrix<Double> uvw = vb.uvw();
  imweight.resize(nvischan, nRow);
  ...
  if (rmode_p == "bwtaper") // See CAS-13021 for bwtaper algorithm details
  {
    // cout << "BriggsCubeWeightor::getWeightUniform bwtaper" << f2_p[0] <<
    // endl;
    if (fracBW == 0.0) {
      throw(AipsError(
          "BriggsCubeWeightor fractional bandwith is not a valid value, 0.0."));
  ...
  for (rownr_t row = 0; row < nRow; row++) {
    for (Int chn = 0; chn < nvischan; chn++) {
      if ((!flag(chn, row)) && (chanMap(chn) > -1)) {
        ...
        //  myfile << vb.rowIds()(0) << " uv " << uvw.column(0) << " loc " <<
        //  pos[0] << ", " << pos[1] << "\n"<< endl; myfile.close();
          ...
          if (gwt > 0) {
            ...
            if (rmode_p ==
              ...
              if (uvDistanceFactor < 1.5)
                uvDistanceFactor = (4.0 - nCellsBW) / (4.0 - 2.0 * nCellsBW);
              imweight(chn, row) /=
                  gwt * f2_p[0][pos[3]] / uvDistanceFactor + d2_p[0][pos[3]];
            } else {
...
void BriggsCubeWeightor::initializeFTMachine(
    const uInt index, const ImageInterface<Complex> &templateimage,
    const RecordInterface &inRec) {
  ...
  ft_p[index] =
      new refim::GridFT(Long(1000000), Int(200), "BOX", 1.0, true, false);
  Int tmpInt;
  inRec.get("freqinterpmethod", tmpInt);
  freqInterpMethod_p =
      static_cast<InterpolateArray1D<Double, Complex>::InterpolationMethod>(
...
void BriggsCubeWeightor::cube2Matrix(const Cube<Bool> &fcube,
                                     Matrix<Bool> &fMat) {
