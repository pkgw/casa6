  ...
  public:
    ...
    */
    ...
    void put(const std::vector<double>  & x)
    {
      if (x.size() != NParam())
      {
	std::string errmsg = "In function ModelDesc::Put expected "+std::to_string(NParam())+" but received "+std::to_string(x.size())+" pars ";
	throw BaseErr(errmsg);
      }
      copytopars(&x[0]);
    }
