...
#include "bnmin1/src/nestedinitial.h"


#ifdef ELLIPSESAMPLE
#include "bnmin1/src/prior_sampler.h"
#include "bnmin1/src/mcmonitor.h"
#endif
...
namespace LibAIR2 {
  ...
  {
    char tstr[120];                                                                                       
    sprintf(tstr, "[%5.2f, %5.2f, %5.2f, %5.2f]", i.TObs[0], i.TObs[1], i.TObs[2], i.TObs[3]); 
		 ...
		 ALMAResBaseList &r)
  {
    ...
    assert(fb.size()==r.ptr_list.size());
    
    std::unique_ptr<dTdLCoeffsSingleInterpolated> res(new dTdLCoeffsSingleInterpolated());

    std::list<ALMAResBase*>::iterator ir=r.ptr_list.begin();
    for(size_t i=0; i<fb.size(); ++i)
    {
